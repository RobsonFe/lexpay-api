...
from oficio.serializer import PrecatorioSerializer, UserLightSerializer
    ...
    def validate(self, data):
        ...
        if status_analise == DueDiligence.StatusAnalise.REJEITADO and not observacoes:
            raise serializers.ValidationError(
                {"observacoes": "O campo de observações tem que ser preenchido"}
            )

        if status_analise == DueDiligence.StatusAnalise.REPACTUADO:
            if not motivo_repactuacao:
                raise serializers.ValidationError(
                    {"motivo_repactuacao": "Informar o motivo."}
                )
            if not observacoes:
                raise serializers.ValidationError(
                    {"observacoes": "Preencher o campo de observações."}
                )
        ...
        if status_analise == DueDiligence.StatusAnalise.APROVADO:
            ...
            if not self.instance:
                raise serializers.ValidationError(
                    "Não é possível criar uma Due Diligence já aprovada."
                )
...
class AnaliseDocumentoSerializer(serializers.ModelSerializer):
    precatorio_detalhes = PrecatorioSerializer(
        source="due_diligence.precatorio", read_only=True
    )
    detalhes_usuario = UserLightSerializer(source="analisado_por", read_only=True)
    due_diligence_detalhes = DueDiligenceSerializer(
        source="due_diligence", read_only=True
    )
