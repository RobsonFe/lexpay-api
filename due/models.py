...
class DueDiligence(models.Model):
    ...
    prioridade = models.CharField(
        max_length=10,
        choices=PrioridadeType.choices,
        default=PrioridadeType.MEDIA,
        db_index=True,
    )
    analista = models.ForeignKey(
        ...
        on_delete=models.PROTECT,
        limit_choices_to={
            "type_user__in": [TypeUserChoices.BROKER, TypeUserChoices.ADMINISTRADOR]
        },
...
class AnaliseDocumento(models.Model):
    ...
    due_diligence = models.ForeignKey(
        DueDiligence, on_delete=models.CASCADE, related_name="analise_documentos"
    )
    documento = models.ForeignKey(
        "oficio.Documento", on_delete=models.PROTECT, related_name="analises_feitas"
    )
