...
from oficio.models import StatusPrecatorioChoices, Precatorio
...
def criar_analises_automaticas(sender, instance, created, **kwargs):
    ...
    with transaction.atomic():
        try:
            precatorio = Precatorio.objects.select_for_update().get(id=instance.precatorio_id)
            ...
            if instance.status_analise == DueDiligence.StatusAnalise.APROVADO:
                if precatorio.status != StatusPrecatorioChoices.DISPONIVEL:
                    ...
                    precatorio.save()
                    logger.info(f"Precatório: {precatorio.numero_processo} - Liberação para venda via DueDiligence: {instance.id}")  
            ...
            logger.error(f"Error: O precatório de id {instance.precatorio_id} não pode ser atualizado pois ele não existe.")